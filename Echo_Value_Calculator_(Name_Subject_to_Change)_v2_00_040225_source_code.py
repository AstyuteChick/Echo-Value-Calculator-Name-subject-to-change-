#OMG OMG OMG I FIXED THE DAMM ER ISSUES!!!
#I'm losing my mind. I've done preliminary tests and the logic works SOOOO much better than before.
#Also the code for ER handling is WAAAAAY more efficient now.
#I mean, the new ER code replaced what was previously 100 or more lines of code with just 25... AND IT IS WAAAAY BETTER AT ITS JOB!!!
#I will do some more testing to be sure and release the final exe very soon now. Also because of a major change in the code's engine, I'm gonna count that as a major update as compared to just balance or character changes/additions.
#Finally, I also added relative scalings for ER to handle weird cases like Jinhsi as opposed to Carlotta.
#Carlotta, for example, NEEDS her liberation. Jinhsi's optimal rotation would include her using her liberation every other rotation.
#So, both these characters will have so ER "targets" so that we don't ignore if someone rolls ER on their Jinhsi pieces.
#Issue is, the code treated Jinhsi's ER target as "IMPORTANT" as Carlotta's. If Carlotta doesn't get her liberation, she doesn't really do damage.
#If Jinhsi doesn't get her liberation - she can continue her rotation by only "losing" a part of the damage from her optimal dmg output.
#I've now solved this issue completely (hopefully) and all the tests seem to have passed! I can't wait for people to experience this new much improved code!
...
def player_data(stat_name, stat_med, char_name, char_stats, net_er, rat_typ):
    ...
    while mechos < rat_typ:
        if er_bank >= 1:
            ...
            er_bank = 0
        useful_stat_val = useful_stat_val + sum(heapq.nlargest(5, [p for p in char_stats if p>=0]))
        mechos = mechos + 1
