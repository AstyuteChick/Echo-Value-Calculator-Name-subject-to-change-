...
class SetMainFrame1(ctk.CTkFrame):
    ...
    def __init__(self, parent, player_char_name: ctk.StringVar, player_calc_mode: ctk.StringVar, instructs: ctk.StringVar) -> None:
        ...
        mode_ddm.pack(side = "left", expand = True)
...
class SetMainFrame12(ctk.CTkFrame):
    ...
    def __init__(self, parent, tot_er: ctk.DoubleVar, er_cons: ctk.StringVar, net_er_disp: ctk.StringVar, er_cons_disp: ctk.StringVar, net_er: ctk.DoubleVar) -> None:
        ...
        tot_er_entry.bind("<Key>", command = lambda event: update_net_er(net_er, tot_er, net_er_disp))
        tot_er_entry.grid(row = 1, column = 0, sticky = "nsw")
        net_er_label = ctk.CTkLabel(
            self, 
            textvariable = net_er_disp
            )
        net_er_label.grid(row = 2, column = 0, sticky = "nsw")

        er_cons_radbut_n = ctk.CTkRadioButton(
            self, 
            text = "Normal [RECOMMENDED]]", 
            variable = er_cons, value = "n", 
            command = lambda: update_er_cons_disp(er_cons_disp, er_cons.get())
            )
        er_cons_radbut_n.grid(row = 0, column = 2, sticky = "nse")
        er_cons_radbut_y = ctk.CTkRadioButton(
            self, 
            text = "Outro Buff (Zhezhi / Yangyang)", 
            variable = er_cons, value = "y", 
            command = lambda: update_er_cons_disp(er_cons_disp, er_cons.get())
            )
        er_cons_radbut_y.grid(row = 1, column = 2, sticky = "nse")
        ...
        er_cons_radbut_x.grid(row = 2, column = 2, sticky = "nse")
        er_cons_selected = ctk.CTkLabel(
            self, 
            textvariable = er_cons_disp
            )
        er_cons_selected.grid(row = 3, column = 2, sticky = "nse")
...
class EchoCell(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        cr_entry.grid(row = 0, column = 1, sticky = "nsw")
        cd_label.grid(row = 1, column = 0, sticky = "nsew")
        cd_entry.grid(row = 1, column = 1, sticky = "nsw")
        atk_label.grid(row = 2, column = 0, sticky = "nsew")
        atk_entry.grid(row = 2, column = 1, sticky = "nsw")
        fatk_label.grid(row = 3, column = 0, sticky = "nsew")
        fatk_entry.grid(row = 3, column = 1, sticky = "nsw")
        hp_label.grid(row = 4, column = 0, sticky = "nsew")
        hp_entry.grid(row = 4, column = 1, sticky = "nsw")
        fhp_label.grid(row = 5, column = 0, sticky = "nsew")
        fhp_entry.grid(row = 5, column = 1, sticky = "nsw")
        def_label.grid(row = 6, column = 0, sticky = "nsew")
        def_entry.grid(row = 6, column = 1, sticky = "nsw")
        fdef_label.grid(row = 7, column = 0, sticky = "nsew")
        fdef_entry.grid(row = 7, column = 1, sticky = "nsw")
        ba_label.grid(row = 8, column = 0, sticky = "nsew")
        ba_entry.grid(row = 8, column = 1, sticky = "nsw")
        ha_label.grid(row = 9, column = 0, sticky = "nsew")
        ha_entry.grid(row = 9, column = 1, sticky = "nsw")
        skill_label.grid(row = 10, column = 0, sticky = "nsew")
        skill_entry.grid(row = 10, column = 1, sticky = "nsw")
        lib_label.grid(row = 11, column = 0, sticky = "nsew")
        lib_entry.grid(row = 11, column = 1, sticky = "nsw")
        er_label.grid(row = 12, column = 0, sticky = "nsew")
        er_entry.grid(row = 12, column = 1, sticky = "nsw")
...
def main():
    ...
    net_er = ctk.DoubleVar(value = 0.0)
