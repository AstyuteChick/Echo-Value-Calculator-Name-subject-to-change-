...
def calc_route():
    ...
    if calc_type=="1":
        ...
        for echo_no in range (0,5):
            print("\n---------- ---------- Echo "+str(echo_no+1)+" ---------- ----------")
            cur_echo_val, char_er=player_data(stat_name, stat_med, char_name, char_stats, char_er, 0)
            echo_eval(cur_echo_val, 0, 0)
            echo_val=echo_val+cur_echo_val
        echo_eval(echo_val/5, 1, 2)
    elif calc_type=="2":
        fb_val, char_er=player_data(stat_name, stat_med, char_name, char_stats, char_er, 1)
        echo_eval(fb_val, 1, 2)
    elif calc_type=="3":
        echo_val, char_er=player_data(stat_name, stat_med, char_name, char_stats, char_er, 0)
...
def game_data(char_name):
    ...
    if char_name=="carlotta" or char_name=="glass girl" or char_name=="small car lotta damage. wait... what?" or char_name=="in the name of the montellis." or char_name=="rin tohsaka" or char_name=="best girl":
        ...
        char_stats=[1, 1, 0.5, 0.25, 0, 0, 0.5*0.75, 0.5*0.2, -115, 0, 0, 0, 0, 0]
        ...
        char_name2=["Encore", "her"]
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.5, 0, 0.5*0.2, 0.5*0.1, -125, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.1, 0.5*0.1, 0.5*0.55, 0.5*0.2, -135, 0, 0, 0, 0, 0]
        ...
        char_stats=[0, 0, 0, 0, 0, 0, 0, 0, -215, 1, 0.5, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.25, 0, 0, 0.5*0.55, -125, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.7, 0, 0, 0.5*0.15, -120, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.05, 0, 0.5*0.6, 0.5*0.25, -125, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0, 0, 0.5*0.5, 0.5*0.3, -150, 0, 0, 0, 0, 0]
    elif char_name=="danjin" or char_name=="darkness falls...":
        char_name2=["Danjin", "her"]
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.05, 0.5*0.25, 0.5*0.25, 0.5*0.3, 0, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0, 0.5*0.7, 0.5*0.15, 0, -130, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.35, 0, 0.5*0.3, 0.5*0.05, -125, 0, 0, 0, 0, 0]
    elif char_name=="lumi" or char_name=="delivery girl" or char_name=="bunny girl" or char_name=="squeakie, protect the cargo!":
        char_name2=["Lumi", "her"]
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.35, 0, 0.5*0.25, 0.5*0.3, -165, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.1, 0, 0.5*0.15, 0.5*0.7, -125, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.05, 0.5*0.6, 0.5*0.15, 0, -135, 0, 0, 0, 0, 0]
        ...
        char_stats=[1, 1, 0.5, 0.25, 0.5*0.3, 0, 0.5*0.2, 0.5*0.25, -125, 0, 0, 0, 0, 0]
        ...
        char_stats=[0, 0, 0, 0, 0, 0, 0, 0, -170, 0, 0, 1, 0.5, 0]
...
def disp_char_data(stat_name, stat_med, char_name2, char_stats):
    ...
    for y in range (0, len(stat_name)-1):
        if char_stats[y]>0:
            print("The reletive value of a median roll ("+str(stat_med[y])+") of "+stat_name[y]+" for "+char_name2[0]+" is: "+str(char_stats[y]*100)+"%")
    if char_stats[8]<0:
        print("The "+stat_name[8]+" requirements for "+char_name2[0]+" are: "+str(-char_stats[8])+"%.")
        print("The reletive value of a median roll ("+str(stat_med[8])+") of "+stat_name[8]+" is fluctuating based on complex factors, and may not be constant even during the evaluation of a single build (option 1)."
...
def player_data(stat_name, stat_med, char_name, char_stats, char_er, rat_typ):
    total_stat_val=0
    if rat_typ==0:
